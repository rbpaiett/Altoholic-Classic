...
local GetItemSubClassInfo = function(c, s) 
    if c and s then return C_Item.GetItemSubClassInfo(c, s) end 
    return "Unknown" 
end
...
[BANK] = {
		...
		GetFreeSlots = function(self)
				local freeSlots, bagType = C_Container.GetContainerNumFreeSlots(-1)

				-- 2026 Anniversary Client Fix: Ensure table and values exist before comparing
				local containerFreeSlots = C_Container.GetContainerFreeSlots(-1)
				local numSlots = C_Container.GetContainerNumSlots(-1)
				
				if containerFreeSlots and #containerFreeSlots > 0 and numSlots then
					local lastFreeSlot = containerFreeSlots[#containerFreeSlots]
					if lastFreeSlot and lastFreeSlot > numSlots then
						freeSlots = C_Container.GetContainerNumFreeSlots(-1) - (lastFreeSlot - numSlots)
					end
				end
		...
		GetCount = function(self, slotID)
				-- Fixed: C_Container.GetContainerItemInfo returns a table; must check if it exists before accessing .stackCount
				local itemInfo = C_Container.GetContainerItemInfo(-1, slotID)
				return itemInfo and itemInfo.stackCount or 0
			end,
