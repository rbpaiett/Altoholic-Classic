...
addon:Controller("AltoholicUI.Calendar", { "AltoholicUI.DateTime", function(dateTime)
    ...
    -- Anniversary TBC 2026 Fix: Get real-time date from server
            local DateInfo = C_DateAndTime.GetCurrentCalendarTime()
            local presentMonth = DateInfo.month
            local presentDay = DateInfo.monthDay -- API uses monthDay
            ...
            while ( day <= numDays ) do
                local isToday = (day == presentDay and month == presentMonth and year == presentYear)
                -- Note: You may need to ensure your Button's Update function accepts an 'isToday' argument
                frame["Day"..buttonIndex]:Update(day, month, year, isDarkened, isToday)
